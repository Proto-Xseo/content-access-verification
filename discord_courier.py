...
def post_file(token, channel, path, content):
    ...
    req = request.Request(f'{DISCORD}/channels/{parse.quote(channel)}/messages', data=b''.join(body), method='POST', headers={'Authorization': f'Bot {token}', 'Content-Type': f'multipart/form-data; boundary={boundary}'})
    try:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(dir=root) as temp:
                ...
                for part_index, part in enumerate(parts):
                    code = post_file(args.token, channels[index % len(channels)], part, f'{source.stem} — part {part_index + 1}/{len(parts)}')
